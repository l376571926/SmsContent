...
//                String verifyCode = getVerifyCode(body);
                String verifyCode = getVerifyCode(body);
//                KLog.e("获取到验证码：" + verifyCode);
                System.out.println("获取到验证码：" + verifyCode);

                if (onVerifyCodeReceiverListener != null) {
                    if (verifyCode != null) {
                        onVerifyCodeReceiverListener.OnVerifyCodeReceiver(verifyCode);
                    }
                }
            }
        } else {
    ...
    private OnVerifyCodeReceiverListener onVerifyCodeReceiverListener;

    public void setOnVerifyCodeReceiverListener(OnVerifyCodeReceiverListener onVerifyCodeReceiverListener) {
        this.onVerifyCodeReceiverListener = onVerifyCodeReceiverListener;
    }

    public interface OnVerifyCodeReceiverListener {
        void OnVerifyCodeReceiver(String verifyCode);
    }
...
//    }
     ...
     * Get verify code from sms body
     *
     * String body = "【小鸣单车】尊敬的用户，您的验证码是 3751 有效期为10分钟，请尽快完成验证操作。如果非本人操作，请尽快向客服反馈！";
     * String body = "【掌淘科技】派助手的验证码：7876";
     *
     * @param smsBody 短信内容
     * @return 3751（7876）
     */
    public static String getVerifyCode(String smsBody) {
        Pattern pattern = Pattern.compile("\\d{4}");
        Matcher matcher = pattern.matcher(smsBody);

        if (matcher.find()) {
            String group = matcher.group();
            System.out.println("--------验证码--------》" + group);
            return group;
        }
